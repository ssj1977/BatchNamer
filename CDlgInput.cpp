...
void CDlgInput::InitInputByCommand(int nCommand)
{
	...
	case IDS_TB_18: //Add End Ext
		...
		m_aInput.Add(item);
		item.Clear();
		item.m_strItemName = IDSTR(IDS_MOVE_PATTERN);
		item.m_nSubCommand = IDS_MOVE_PATTERN;
		item.m_strLabel1 = IDSTR(IDS_PATTERN_FIND);
		if (nCommand == IDS_TB_02)	item.m_strLabel2 = IDSTR(IDS_INSERT_BYPOS_FRONT);
		else						item.m_strLabel2 = IDSTR(IDS_INSERT_BYPOS_BACK);
		m_aInput.Add(item);
		item.Clear();
		item.m_strItemName = IDSTR(IDS_MOVE_BRACKET);
		item.m_nSubCommand = IDS_MOVE_BRACKET;
		item.m_strLabel1 = IDSTR(IDS_BRACKET1);
		item.m_strLabel2 = IDSTR(IDS_BRACKET2);
		m_aInput.Add(item);
		item.Clear();
		item.m_strItemName = IDSTR(IDS_MOVE_POS);
		item.m_nSubCommand = IDS_MOVE_POS;
		item.m_strLabel1 = IDSTR(IDS_POS_1);
		item.m_strLabel2 = IDSTR(IDS_POS_2);
		m_aInput.Add(item);
		item.Clear();
		item.m_strItemName = IDSTR(IDS_MOVE_POS_REVERSE);
		item.m_nSubCommand = IDS_MOVE_POS_REVERSE;
